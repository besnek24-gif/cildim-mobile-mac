...
REPORT_DIR="reports/ecz4_scan_visibility_pack_v43_retry_$STAMP"
mkdir -p "$REPORT_DIR" stable_snapshots backups/ecz4_v43 local_demo_data

cp app/index.tsx "backups/ecz4_v43/index_before_v43_retry_$STAMP.tsx"
cp app/_layout.tsx "backups/ecz4_v43/layout_before_v43_retry_$STAMP.tsx"
cp local_demo_data/products_v37.ts "backups/ecz4_v43/products_v37_before_v43_retry_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/decision_v38.ts "backups/ecz4_v43/decision_v38_before_v43_retry_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/analysis_v39.ts "backups/ecz4_v43/analysis_v39_before_v43_retry_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/scan_v40.ts "backups/ecz4_v43/scan_v40_before_v43_retry_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/ux_guard_v41.ts "backups/ecz4_v43/ux_guard_v41_before_v43_retry_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/test_readiness_v42.ts "backups/ecz4_v43/test_readiness_v42_before_v43_retry_$STAMP.ts" 2>/dev/null || true
...
if "ECZ4_PRE_TEST_HARDENING_PACK_V42" not in s and "ECZ4_SCAN_VISIBILITY_PACK_V43" not in s:
    print("V42/V43 marker bulunamadı. Aktif dosya beklenen hatta değil.")
    raise SystemExit(1)
...
pattern = re.compile(r'        \{screen === "scan" && \(\n.*?\n        \)}\n\n(?=        \{screen === "compare" && \()', re.DOTALL)
s2, count = pattern.subn(scan_block, s, count=1)

if count != 1:
    print("Tara ekran bloğu regex ile bulunamadı. Scan/compare çevresi:")
    lines = s.splitlines()
    for i, line in enumerate(lines, start=1):
        if 'screen === "scan"' in line or 'screen === "compare"' in line:
            start = max(1, i - 5)
...
p.write_text(s)
print("V43 scan visibility retry patch uygulandı.")
PY

{
echo "=== V43 RETRY SOURCE CHECK ==="
...
echo ""
echo "=== IOS EXPORT ==="
rm -rf "dist/ecz4_scan_visibility_pack_v43_retry_$STAMP"
npx expo export --platform ios --output-dir "dist/ecz4_scan_visibility_pack_v43_retry_$STAMP" --clear

echo ""
echo "=== BUNDLE CHECK ==="
BUNDLE=$(find "dist/ecz4_scan_visibility_pack_v43_retry_$STAMP" -type f -name "*.js" | head -1)
...
tar -czf "stable_snapshots/ECZ4_SCAN_VISIBILITY_PACK_V43_RETRY_PASS_$STAMP.tar.gz" app local_demo_data app.json package.json .easignore quarantine "$REPORT_DIR"

echo ""
echo "=== FINAL ==="
echo "PASS: ECZ4 scan visibility pack v43 retry kaynak/export/bundle temiz ve snapshot alındı."
ls -lh "stable_snapshots/ECZ4_SCAN_VISIBILITY_PACK_V43_RETRY_PASS_$STAMP.tar.gz"
