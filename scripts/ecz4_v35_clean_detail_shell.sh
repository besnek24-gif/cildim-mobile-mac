...
REPORT_DIR="reports/ecz4_clean_detail_shell_v35_$STAMP"
mkdir -p "$REPORT_DIR" stable_snapshots backups/ecz4_v35_clean

cp app/index.tsx "backups/ecz4_v35_clean/index_before_v35_clean_$STAMP.tsx"
cp app/_layout.tsx "backups/ecz4_v35_clean/layout_before_v35_clean_$STAMP.tsx"
...
const PRODUCTS: DemoProduct[] = [
  {
    ...
    detail: "Bu örnek ürün alanı, ileride gerçek ürün bilgisiyle beslenecek detay panelini temsil eder.",
  },
  {
    id: "spf",
    ...
    detail: "Gündüz önerilerinde koruma adımının nasıl anlatılacağını göstermek için eklendi.",
  },
  {
    id: "cleanser",
    name: "Arındırıcı Jel",
    ...
    detail: "Rutin başlangıcındaki temizlik adımı için sade ürün anlatımı demo olarak gösterilir.",
  },
];
...
const ROUTINE = [
  {
    ...
    detail: "Sabah akışı hızlı, sade ve koruma odaklı anlatılır.",
  },
  {
    id: "evening",
    time: "Akşam",
    ...
    detail: "Akşam akışı temizleme ve bariyer desteği üstünden kurulur.",
  },
];

const MODULES = [
  { key: "analysis" as const, title: "Cilt Analizi", badge: "Demo", subtitle: "Örnek analiz sonucu ve bakım öncelikleri." },
  { key: "products" as const, title: "Ürün Önerileri", badge: "Demo", subtitle: "Seçilebilir örnek ürün kartları." },
  { key: "routine" as const, title: "Rutinim", badge: "Demo", subtitle: "Sabah-akşam seçilebilir rutin akışı." },
  { key: "compare" as const, title: "Karar Rehberi", badge: "Demo", subtitle: "İki ürünü sade farklarla kıyaslayan alan." },
  { key: "profile" as const, title: "Profil", badge: "Misafir", subtitle: "Kişisel alan sonra kontrollü açılacak." },
];

export default function Index() {
  const [active, setActive] = useState<SectionKey>("home");
  const [selectedProductId, setSelectedProductId] = useState<string>("barrier");
  ...
  const selectedRoutine = useMemo(
    () => ROUTINE.find((item) => item.id === selectedRoutineId) ?? ROUTINE[0],
    [selectedRoutineId],
  );
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          ...
          <Text style={styles.subtitle}>Detaylı demo iskelet</Text>
          <Text style={styles.note}>
            Ürün, rutin, analiz ve karar alanları güvenli demo verilerle çalışıyor.
          </Text>

          <View style={styles.markerBox}>
            <Text style={styles.marker}>ECZ4_CLEAN_DETAIL_SHELL_V35</Text>
          </View>
        </View>

        <View style={styles.navRow}>
          <NavPill label="Ana" active={active === "home"} onPress={() => go("home")} />
          {MODULES.map((item) => (
            <NavPill
              key={item.key}
        ...
        {active === "home" && (
          <>
            ...
            <View style={styles.highlightCard}>
              ...
              <Text style={styles.highlightTitle}>Sade rutin daha iyi anlaşılır.</Text>
              <Text style={styles.highlightText}>
                Cilt bakımında önce ihtiyaç anlaşılır, sonra ürün ve rutin dili sadeleştirilir.
              </Text>
            </View>
            ...
            <View style={styles.grid}>
              {MODULES.map((item) => (
                <Pressable key={item.key} style={styles.moduleCard} onPress={() => go(item.key)}>
        ...
        {active === "analysis" && (
          <View style={styles.detailCard}>
            ...
            <InfoBox
              title="Analiz yorumu"
              text="Bu alan gerçek ölçüm almadan örnek bakım önceliklerini gösterir. Amaç sonuç dilini sağlamlaştırmak."
            />
        ...
        {active === "products" && (
          <View style={styles.detailCard}>
            ...
            <Text style={styles.detailSubtitle}>
              Kartlara basınca güvenli detay paneli açılır.
            </Text>
        ...
        {active === "routine" && (
          <View style={styles.detailCard}>
            ...
            <Text style={styles.detailSubtitle}>
              Sabah ve akşam bloklarına basınca açıklama değişir.
            </Text>

            {ROUTINE.map((block) => {
              const selected = block.id === selectedRoutineId;
              return (
                <Pressable
                  key={block.id}
                  style={[styles.routineBlock, selected && styles.routineBlockActive]}
                  onPress={() => setSelectedRoutineId(block.id)}
                >
            ...
            <InfoBox title={selectedRoutine.time} text={selectedRoutine.detail} />
        ...
        {active === "compare" && (
          <View style={styles.detailCard}>
            ...
            <Text style={styles.detailSubtitle}>
              Ürün farkları sade cümleyle anlatılır.
            </Text>
        ...
        {active === "profile" && (
          <View style={styles.detailCard}>
            ...
            <Text style={styles.detailSubtitle}>
              Kişisel alan şimdilik misafir görünümünde tutulur.
            </Text>
...
const styles = StyleSheet.create({
  ...
  routineBlockActive: { borderColor: "#B07A3A", backgroundColor: "#FFFDF8" },
...
{
echo "=== V35 CLEAN SOURCE CHECK ==="
find app -type f | sort
grep -RIn "ECZ4_CLEAN_DETAIL_SHELL_V35" app
...
echo ""
echo "=== IOS EXPORT ==="
rm -rf "dist/ecz4_clean_detail_shell_v35_$STAMP"
npx expo export --platform ios --output-dir "dist/ecz4_clean_detail_shell_v35_$STAMP" --clear

echo ""
echo "=== BUNDLE CHECK ==="
BUNDLE=$(find "dist/ecz4_clean_detail_shell_v35_$STAMP" -type f -name "*.js" | head -1)
...
grep -q "ECZ4_CLEAN_DETAIL_SHELL_V35" "$BUNDLE"
...
tar -czf "stable_snapshots/ECZ4_CLEAN_DETAIL_SHELL_V35_PASS_$STAMP.tar.gz" app app.json package.json .easignore quarantine "$REPORT_DIR"

echo ""
echo "=== FINAL ==="
echo "PASS: ECZ4 clean detail shell v35 kaynak/export/bundle temiz ve snapshot alındı."
ls -lh "stable_snapshots/ECZ4_CLEAN_DETAIL_SHELL_V35_PASS_$STAMP.tar.gz"
