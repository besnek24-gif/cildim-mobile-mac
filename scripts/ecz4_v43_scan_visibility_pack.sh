...
REPORT_DIR="reports/ecz4_scan_visibility_pack_v43_$STAMP"
mkdir -p "$REPORT_DIR" stable_snapshots backups/ecz4_v43 local_demo_data

cp app/index.tsx "backups/ecz4_v43/index_before_v43_$STAMP.tsx"
cp app/_layout.tsx "backups/ecz4_v43/layout_before_v43_$STAMP.tsx"
cp local_demo_data/products_v37.ts "backups/ecz4_v43/products_v37_before_v43_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/decision_v38.ts "backups/ecz4_v43/decision_v38_before_v43_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/analysis_v39.ts "backups/ecz4_v43/analysis_v39_before_v43_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/scan_v40.ts "backups/ecz4_v43/scan_v40_before_v43_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/ux_guard_v41.ts "backups/ecz4_v43/ux_guard_v41_before_v43_$STAMP.ts" 2>/dev/null || true
cp local_demo_data/test_readiness_v42.ts "backups/ecz4_v43/test_readiness_v42_before_v43_$STAMP.ts" 2>/dev/null || true
...
if "ECZ4_PRE_TEST_HARDENING_PACK_V42" not in s:
    print("V42 marker bulunamadı. Önce V42 aktif olmalı.")
    raise SystemExit(1)
...
# Tara ekranı içeriğini daha görünür seçim kartlarına çevir
old_scan_start = '''            <Text style={styles.smallLabel}>Akış tipi</Text>
...
new_scan_start = '''            <View style={styles.scanSelectionSummary}>
...
if old_scan_start not in s:
    print("Tara seçim bloğu beklenen formatta bulunamadı.")
    raise SystemExit(1)

s = s.replace(old_scan_start, new_scan_start)
...
p.write_text(s)
print("V43 scan visibility patch uygulandı.")
PY

{
echo "=== V43 SOURCE CHECK ==="
...
echo ""
echo "=== IOS EXPORT ==="
rm -rf "dist/ecz4_scan_visibility_pack_v43_$STAMP"
npx expo export --platform ios --output-dir "dist/ecz4_scan_visibility_pack_v43_$STAMP" --clear

echo ""
echo "=== BUNDLE CHECK ==="
BUNDLE=$(find "dist/ecz4_scan_visibility_pack_v43_$STAMP" -type f -name "*.js" | head -1)
...
tar -czf "stable_snapshots/ECZ4_SCAN_VISIBILITY_PACK_V43_PASS_$STAMP.tar.gz" app local_demo_data app.json package.json .easignore quarantine "$REPORT_DIR"

echo ""
echo "=== FINAL ==="
echo "PASS: ECZ4 scan visibility pack v43 kaynak/export/bundle temiz ve snapshot alındı."
ls -lh "stable_snapshots/ECZ4_SCAN_VISIBILITY_PACK_V43_PASS_$STAMP.tar.gz"
