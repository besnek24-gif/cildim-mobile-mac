...
REPORT_DIR="reports/ecz4_v48_full_runtime_import_fix_$STAMP"
...
cp app/index.tsx "backups/ecz4_v48_runtime_fix/index_before_full_import_fix_$STAMP.tsx"
cp -R local_demo_data "backups/ecz4_v48_runtime_fix/local_demo_data_before_full_import_fix_$STAMP"
...
insert_anchor = 'import {\n  TEST_CHECKLIST_V43,\n  TEST_SUMMARY_V43,\n} from "../local_demo_data/test_readiness_v42";'

if insert_anchor not in s:
    print("Beklenen test_readiness import bloğu bulunamadı. İlk 80 satır:")
    for i, line in enumerate(s.splitlines()[:80], 1):
        print(f"{i}: {line}")
    raise SystemExit(1)
...
# V43 isimleri app içinde kullanılıyor; test_readiness_v42 dosyasına V43 alias export ekle.
...
print("V48 full runtime import fix uygulandı.")
PY

{
echo ""
echo "=== V48 FULL RUNTIME FIX SOURCE CHECK ==="
...
grep -RInE 'product_detail_v44|routine_plan_v45|analysis_scan_polish_v46|profile_v47|TEST_SUMMARY_V43|TEST_CHECKLIST_V43' app local_demo_data
...
echo ""
echo "=== IOS EXPORT ==="
rm -rf "dist/ecz4_v48_full_runtime_import_fix_$STAMP"
npx expo export --platform ios --output-dir "dist/ecz4_v48_full_runtime_import_fix_$STAMP" --clear

echo ""
echo "=== BUNDLE CHECK ==="
BUNDLE=$(find "dist/ecz4_v48_full_runtime_import_fix_$STAMP" -type f -name "*.js" | head -1)
...
tar -czf "stable_snapshots/ECZ4_V48_FULL_RUNTIME_IMPORT_FIX_PASS_$STAMP.tar.gz" app local_demo_data app.json package.json .easignore quarantine "$REPORT_DIR"

echo ""
echo "=== FINAL ==="
echo "PASS: ECZ4 V48 full runtime import fix kaynak/export/bundle temiz ve snapshot alındı."
ls -lh "stable_snapshots/ECZ4_V48_FULL_RUNTIME_IMPORT_FIX_PASS_$STAMP.tar.gz"
