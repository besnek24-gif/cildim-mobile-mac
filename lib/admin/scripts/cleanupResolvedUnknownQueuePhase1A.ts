/**
 * cleanupResolvedUnknownQueuePhase1A.ts
 * ─────────────────────────────────────────────────────────────────────────────
 * Phase 1A — Queue cleanup utility.
 *
 * PURPOSE:
 *   The Phase 1A insert script (batchInsertProductionUnknownsPhase1A.ts) added
 *   17 LOW-risk ingredients to ingredients_master + ingredient_aliases but
 *   intentionally did NOT touch ingredient_unknown_queue. Those queue rows
 *   therefore still appear as `pending` even though the resolver can now
 *   resolve them. This script flips `resolution_status` to "resolved" for
 *   ONLY those queue rows whose normalized_name is now resolvable through
 *   one of the 17 Phase 1A canonicals or their registered aliases.
 *
 ...
 *     (intersection of master canonicals + aliases for the 17 Phase 1A items)
 *   - Only flips rows whose current resolution_status = "pending"
 *
 * NORMALIZED-FORM SOURCE OF TRUTH:
 *   The 17 canonical names below are the EXACT same ones inserted by
 *   batchInsertProductionUnknownsPhase1A.ts. We deliberately duplicate them
 ...
 *       lib/admin/scripts/cleanupResolvedUnknownQueuePhase1A.ts
 *
...
// ── Phase 1A canonicals (must match batchInsertProductionUnknownsPhase1A.ts) ──
const PHASE_1A_CANONICALS: readonly string[] = [
  "perlite",
  "sodium citrate",
...
async function fetchPhase1AMasterIds(
  sb: SupabaseClient,
...
async function main() {
  console.log("─────────────────────────────────────────────────────────────");
  console.log(" cleanupResolvedUnknownQueuePhase1A");
  console.log(` Supabase: ${process.env.EXPO_PUBLIC_SUPABASE_URL}`);
  console.log(` MODE    : ${DRY_RUN ? "DRY-RUN  (no writes)" : "LIVE  (writes enabled)"}`);
  console.log(` Phase 1A canonicals declared: ${PHASE_1A_CANONICALS.length}`);
  console.log("─────────────────────────────────────────────────────────────\n");

  const sb = createLeanSupabase();

  // ── Step 1: Resolve Phase 1A canonicals → ingredient ids ────────────────────
  const normalizedCanonicals = PHASE_1A_CANONICALS.map((c) => normalizeForLookup(c));
  const masterRows           = await fetchPhase1AMasterIds(sb, normalizedCanonicals);
  ...
  console.log(" Phase 1A → ingredients_master lookup:");
  console.log(`   present in master : ${masterRows.length} / ${PHASE_1A_CANONICALS.length}`);
  ...
  console.log(`   pending queue rows resolvable via Phase 1A : ${pendingMatches.length}`);
  console.log();

  // ── Step 4: Group + report planned updates ──────────────────────────────────
  // Group every matched queue row under the Phase 1A canonical it now belongs to.
  // We do this purely for the human-readable report — the actual UPDATE filter
  // is by normalized_name, not by canonical, so this grouping has no side effect.
  const aliasNormToCanon = new Map<string, string>();
  // First, all canonicals map to themselves
  for (const c of presentMasterCanonSet) aliasNormToCanon.set(c, c);
  // Re-fetch alias rows with their ingredient_id so we can map alias→canonical.
  // (We already have aliasNorms above, but that lost the ingredient_id mapping.)
  {
  ...
  for (const row of pendingMatches) {
    ...
    if (!canon) {
      // Defensive: should not happen, since pendingMatches came from resolvedSet
      // (which equals canonicals ∪ aliasNorms). But guard anyway.
      skipped.push({
        raw_name:        row.raw_name,
        normalized_name: row.normalized_name,
        reason:          "could not map to a Phase 1A canonical (defensive skip)",
      });
      continue;
    }
    planned.push({
  ...
  // Group printable plan by canonical
  ...
  // Stable order: same as PHASE_1A_CANONICALS
  ...
  console.log("─────────────────────────────────────────────────────────────");
  console.log(`   Phase 1A canonicals declared          : ${PHASE_1A_CANONICALS.length}`);
  console.log(`   Phase 1A canonicals present in master : ${masterRows.length}`);
  console.log(`   Phase 1A canonicals missing in master : ${missingFromMaster.length}`);
  ...
  // Update one row at a time, scoped to (normalized_name, status='pending') so
  // that we (a) only ever touch rows we expected to touch and (b) never flip a
  // row that something else has already moved out of "pending".
  for (const p of planned) {
